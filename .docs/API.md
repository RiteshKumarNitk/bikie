...
| `GET /api/trips` | `tab` (upcoming/weekend/adventure/road-trip/international/guided-tour/completed), `destination` (a `Destination` slug), `difficulty`, `from`/`to` (startDate range) | `{ trips: TripSummaryDTO[] }`. ADR-068: `destination` matches the selected catalog destination's *name* case-insensitively against the freeform `destinationName` riders actually see (OR'd with the legacy relational link) — matching only the relation, as before, matched zero rides created since ADR-037 stopped populating it. |
| `GET /api/trips/[slug]` | — | `{ trip: TripDetailDTO }`. Public/unauthenticated (discovery) — but ADR-065 redacts `meetingPoint`/`meetingLat`/`meetingLng`/`members` to `null`/absent unless the caller is the organizer, an approved member, or an admin, mirroring the SOS PII-redaction pattern (ADR-045). |
...
|---|---|---|---|
| `/api/trips` | POST | Membership required | ADR-068: rate-limited (10/hour). Body: `{ title, description, type, difficulty, seatsTotal, meetingPoint?, meetingLat?, meetingLng?, destinationName?, destinationId?, startDate, endDate, imageUrl?, price? }` (ADR-037: `destinationName` is organizer-typed free text, no longer required to match the curated `Destination` catalog; `destinationId` kept optional for backward compat only. `meetingLat`/`meetingLng` must be provided together. ADR-068: `endDate` must be after `startDate`). `type` ∈ `WEEKEND\|ADVENTURE\|ROAD_TRIP\|INTERNATIONAL\|GUIDED_TOUR`, `difficulty` ∈ `EASY\|MODERATE\|HARD` (default `MODERATE`), `price` defaults to `0` (free/community ride). Slug auto-generated from title (deduped with a numeric suffix). Returns `{ trip: TripSummaryDTO }`, 201. |
| `/api/trips/[slug]` | PATCH | Session, organizer only | Partial update, same fields as create. ADR-068: re-validates the effective date range against the trip's own current dates when only one of `startDate`/`endDate` is sent (`400 INVALID_DATES` if still inverted); rejects shrinking `seatsTotal` below the already-approved count instead of leaving `seatsLeft` inconsistent (`400 SEATS_BELOW_APPROVED`) and recomputes `seatsLeft` alongside any accepted `seatsTotal` change. A date change notifies every approved member directly (`TRIP_RESCHEDULED`); any other field change keeps the existing system-chat-message-only behavior. `404 NOT_FOUND` / `403 FORBIDDEN`. |
| `/api/trips/[slug]/requests` | POST | Session | ADR-068: rate-limited (20/10min). Body: `{ message? }`. 404 if trip not found, 400 if trip isn't `UPCOMING` or caller is the organizer, 400 if no seats left, 409 if already requested (PENDING/APPROVED). Re-requesting after a REJECTED/CANCELLED decision is allowed (upserts back to PENDING). Returns `{ success: true }`, 201. |
...
| `/api/trips/[slug]/leave` | POST | Session | Rider withdraws (PENDING or APPROVED → `CANCELLED`). If was `APPROVED`, increments `seatsLeft` back and notifies the organizer directly (ADR-068; previously only a system chat message). 400 if not currently a participant. |
| `/api/trips/[slug]/cancel` | POST | Session | ADR-067. Organizer or admin only. `{reason?}` (≤300 chars). Transitions the whole `Trip` `UPCOMING → CANCELLED` (idempotent — a second call returns `409 NOT_UPCOMING`), locks the Ride Room conversation, posts a system message, and notifies every approved member and still-pending requester. `404 TRIP_NOT_FOUND` / `403 FORBIDDEN` / `409 NOT_UPCOMING`. |
...
| `/api/partner/profile` | GET/PUT | Partner business profile (`businessName`, `type`, `city`, `description`, `addressLine`/`area`/`pincode`, `latitude`/`longitude`, `governmentIdType`/`governmentIdNumber` — ADR-036, superseding the old `aadhaarNumber` field). PUT now also accepts `isGeneralResponder` (ADR-044). **ADR-053/055**: **both GET and PUT require a session + `accountType: SERVICE_PROVIDER`** (GET also allows ADMIN, for support) — never a membership. Creating/reading your own business profile is what a provider does *before* buying a Service Provider membership, so gating either on `requirePartnerCapability` locks them out of onboarding; ADR-055 moved GET off that gate for this reason. A `RIDER`-accountType account gets `403 {error: "WRONG_ACCOUNT_TYPE"}` and must go through `/account-type-request`. PUT 409s with `{error: "NOT_EDITABLE", status}` while `PENDING_VERIFICATION`/`APPROVED`/`SUSPENDED` (must go through an explicit transition first). |
...
| `/api/partner/sos/nearby` | GET | ADR-044. `?lat=&lng=` required. `{ requests: PartnerNearbyRequestDTO[] }` — open (unassigned), type-eligible SOS alerts within 25km. Empty (not an error) if the partner is unverified or offline. ADR-045: also excludes alerts this partner has already offered on or declined. ADR-064: also excludes every Red/Emergency alert unconditionally — a Service Provider can never see or accept one, whether dispatched to or browsing in. |
| `/api/partner/sos/pending` | GET | ADR-062. `{ offers: PartnerPendingOfferDTO[] }` — this partner's own outstanding offers (OFFERED, not yet accepted/rejected/withdrawn/expired) on alerts still open. Invisible to `/nearby` (excluded once responded to, ADR-045) and to `/active` (no `SOSSession` exists until the rider accepts) — without this list an offer had no visible home anywhere between those two states. |
...
| Route | Method | Notes |
|---|---|---|
| `/api/partners/nearby` | GET | `?lat=&lng=&type=&eligibleOnly=` (type/eligibleOnly optional) → `{ partners: NearbyPartnerRow[] }`, verified/geotagged partners within 25km, sorted nearest-first. `eligibleOnly=true` (ADR-066) additionally requires `isAvailable` + an active `PartnerMembership` — the same rule SOS dispatch uses; default (omitted) keeps the general directory behavior (shows an offline/lapsed partner with a badge rather than hiding them). No session required — IP rate-limited (20/min), not membership-gated (queries our own table, no paid external API involved). Backs the "Service providers near you" map on `/roadside-assistance` (web, `eligibleOnly=true`) and the `/partners` screen (mobile, `eligibleOnly=true`) — both Leaflet/flutter_map + OpenStreetMap, no Google Maps. |

## SOS Emergency (membership required — admins bypass) — ADR-033 staged Community Emergency Response System

| Route | Method | Notes |
|---|---|---|
| `/api/sos/alerts` | GET/POST | List active alerts / send a new one. Open to every membership-holding session — ADR-046b's dual-capability model means there's no such thing as an account with no Rider standing, so a Service Provider (who is always also a Rider) can still raise/browse their own SOS same as anyone; the Partner Dashboard simply never links here, using `/api/partner/sos/*` + the singular `GET /api/sos/alerts/[id]` for its own accept/decline flow instead. **GET** `?lat=&lng=` (ADR-042) — a GPS radius (25km, plain Haversine) around the caller, replacing the old `?city=` exact-string match; non-admin callers without both get `400 { error: "LOCATION_REQUIRED", message }` (ADMIN sees every active alert network-wide, unfiltered, same as before). `SOSAlertDTO` now also includes `severity` (server-derived from `type`, never client input), `escalationTier`, `currentRadiusMeters`, `assignedHelperId`, and (ADR-038) `placeName`/`area`/`formattedAddress` — a best-effort reverse-geocoded address resolved once from `latitude`/`longitude` at creation time (null if the lookup failed/timed out; every reader falls back to `city`/raw coordinates in that case). ADR-044 additionally joins `riderVehicleType`/`riderVehicleBrand`/`riderVehicleModel` from the reporter's `RiderProfile` (all nullable — most riders never fill this in). ADR-059 adds `riderVehicleRegistrationNumber` alongside them, same nullability/redaction treatment — also feeds the DLT "BIKIE_SR" SMS template's Vehicle Registration Number slot (falls back to `"N/A"` there specifically, never left blank, since an empty `##alphanumeric##` value risks MSG91's DLT content filter rejecting the whole message). ADR-045: `userPhone`/`userEmail`/`latitude`/`longitude`/`placeName`/`area`/`formattedAddress` are `null` for any viewer who is not the alert's reporter, its assigned helper, or an admin — see `.docs/SOS.md` §6. This GET also attaches a server-computed `distanceMeters` per alert now (previously computed internally for radius filtering, then discarded). `requireMembership()` returns `403 { error: "MEMBERSHIP_REQUIRED", message }` if the caller has no active membership. **POST** still takes `city` (free text, stored for display only now — no longer used to decide who can see the alert) and runs `SOSDispatchService.fanOut`, which immediately covers emergency contacts + optional `SOS_EMERGENCY_SERVICES_*` + tier-1 nearby riders (5km) **+ tier-1 eligible Service Providers at that same 5km radius, together with the riders, not a later fallback tier (ADR-047)** — **except for a Red/Emergency alert (`ACCIDENT`/`LIFE_THREATENING`/`MEDICAL`), which never resolves any Service Provider candidates at all, on this or any later radius-widening tick (ADR-064)**; only an Amber/Assistance alert reaches both pools. The old `SERVICE_PROVIDERS` escalation tier this paragraph used to describe no longer exists — see `.docs/SOS.md` §2/§4 for the full severity-gated dispatch model. Response `dispatch` still carries `channels`/`escalatedToAdmins` (ADR-030's zero-recipient guarantee, preserved — see ADR-033 for the exact trigger-shape change) and per-channel counts. |
...
| `/api/sos/alerts/[id]/offer` | POST | Helper taps "I'm Coming." Optional `{latitude, longitude, message}` — distance/ETA computed straight-line if location given. Replaces `/respond`. When the caller has a Partner profile (`partnerStatus != null` — not `role`, which no longer indicates Partner capability, ADR-053), additionally gated on: profile not admin-`SUSPENDED` (`403 NOT_VERIFIED` — despite the name, verification itself is never checked, only suspension); **active ₹99/month Service Provider membership** (`403 MEMBERSHIP_REQUIRED`, ADR-056); available + type-eligible (`partnerMatchesAlertType`) + not already at capacity (1 concurrent active session as helper): `409` (`PARTNER_OFFLINE`/`CATEGORY_MISMATCH`/`AT_CAPACITY`). Belt-and-suspenders — SOS dispatch fan-out (`findEligiblePartnersNearPoint`) already only notifies partners with an active membership in the first place; this is the second check for a caller who reached the alert ID some other way (forwarded link, stale membership since dispatch). |
...
| `/api/membership/checkout` | POST | ADR-043. Body: `{ planId }`. Creates a real Razorpay order, priced server-side from the plan (never a client-supplied amount). Returns `{ razorpayConfigured: false }` if Razorpay isn't configured (`RAZORPAY_KEY_ID`/`RAZORPAY_KEY_SECRET` unset) — the client's simulated-checkout fallback handles that case entirely on its own, without calling `/purchase` any differently. **ADR-069**: when Razorpay is unconfigured **and** `NODE_ENV === "production"`, returns `503 { error: "PAYMENTS_UNAVAILABLE" }` instead — the no-charge simulated path is dev/preview only. Configured: `{ razorpayConfigured: true, order: { orderId, amount, currency, keyId }, plan: { id, name, price } }`. |
| `/api/membership/purchase` | POST | ADR-043. Two mutually exclusive body shapes, resolved by whether Razorpay is configured server-side (not a client choice): **(a)** `{ planId, paymentId }` — the pre-Razorpay simulated-checkout shape, only accepted while Razorpay is unconfigured; `paymentId` is a client-generated dummy string (`DUMMY-<uuid>`). **(b)** `{ planId, razorpayOrderId, razorpayPaymentId, razorpaySignature }` — required once Razorpay is configured; the route verifies `HMAC-SHA256(orderId\|paymentId, keySecret) === signature` server-side before creating a membership, `400 { error: "PAYMENT_VERIFICATION_FAILED" }` if it doesn't match, `400 { error: "PAYMENT_VERIFICATION_REQUIRED" }` if shape (a) is sent while Razorpay is configured. Mirrors `components/membership/PaymentModal.tsx`. Returns `{ membership }`. **ADR-069**: (i) replaying a payment reference (`paymentId` or `razorpayOrderId`) that already activated a membership is idempotent — returns that same `{ membership }`, no new row, no re-fired SMS (DB `@unique` on both columns is the guarantee); (ii) `409 { error: "ALREADY_ACTIVE_MEMBERSHIP" }` if the caller already holds an active membership and sends a *new* payment; (iii) shape (a) in production while Razorpay is unconfigured → `503 { error: "PAYMENTS_UNAVAILABLE" }`. **Mobile still only ever sends shape (a)** — no native Razorpay integration yet, so mobile purchases will fail with `PAYMENT_VERIFICATION_REQUIRED` once real keys are set; see ADR-043. **ADR-058**: on success, fires the DLT-approved "BIKIE_Sub" SMS confirmation (fire-and-forget, never fails the request) to the purchasing user's phone number — Rider-only, never sent for a Service Provider (partner) membership purchase. |
...
| Route | Method | Notes |
|---|---|---|
| `/api/partner-membership/active` | GET | Auth required (session only — no `accountType`/capability check, deliberately: this is what lets any `/partner/**` page ask "does this account have an active membership" before it necessarily has one). `{ membership: PartnerMembershipDTO \| null }` |
| `/api/partner-membership/plans` | GET | Public. `{ plans: PartnerMembershipPlanDTO[] }` — **ADR-056**: the live offering is one active plan, "Service Provider Membership", ₹99/`durationDays: 30`. The `legacy-free-partner-plan` row from `20260811100000_partner_membership_model`'s backfill (`price: 0`, 100-year duration) still exists and its grandfathered `PartnerMembership` rows stay valid, but it's `isActive: false` so it no longer appears here or is purchasable by a new signup. |
| `/api/partner-membership/checkout` | POST | Body: `{ planId }`. A plan whose server-side `price` is `0` (free tier) returns `{ razorpayConfigured: false, free: true }` without ever reaching Razorpay — the client purchases directly with no payment step. Otherwise identical to `/api/membership/checkout`, including the ADR-069 `503 { error: "PAYMENTS_UNAVAILABLE" }` production gate (a free plan is checked first and is never gated). |
| `/api/partner-membership/purchase` | POST | Same two body shapes as `/api/membership/purchase`, plus: if the resolved plan's server-side `price` is `0`, the membership activates immediately regardless of what the client sent (never a client-asserted "this is free" flag). **ADR-069**: same idempotent-replay, `409 ALREADY_ACTIVE_MEMBERSHIP` (the free-tier "Activate" path hits this guard too), and production `503 PAYMENTS_UNAVAILABLE` behavior as the Rider route. |
| `/api/admin/partner-membership/plans` | GET/POST | Role: ADMIN. |
| `/api/admin/partner-membership/plans/[id]` | PATCH/DELETE | Role: ADMIN. |

## Membership billing / invoices — ADR-070

One immutable `MembershipInvoice` per membership activation (both account types), with a
purchase-time snapshot of amount/currency/plan/duration/membership-window/payer identity — a
later admin change to a plan never alters a historic invoice. Every route is scoped to
`session.user.id`; a client-supplied user id is never accepted.

| Route | Method | Notes |
|---|---|---|
| `/api/billing/history` | GET | Auth required. `{ invoices: InvoiceSummaryDTO[] }` — the caller's own membership payments, newest first, covering both account types (invoices from before an account-type change are still theirs). |
| `/api/billing/invoices/[id]` | GET | Auth required. `{ invoice: InvoiceDetailDTO }`. **`404` for both a non-existent id and an id belonging to another user** (`getInvoice` returns `null` in both cases) — invoice ids can't be probed by swapping the path segment. |
| `/api/billing/invoices/[id]/receipt` | GET | Auth required. Same invoice as an inline, printable **`text/html`** document (browser "Save as PDF"; opens in a mobile webview). Same ownership rule — a missing / someone-else's id is a plain `404`. No PDF library, no stored file — rendered on demand from the stored snapshot. |

Invoice creation is a side effect of `POST /api/membership/purchase` / `POST /api/partner-membership/purchase` (after activation, before the confirmation SMS). It is idempotent — a replayed `/purchase` yields the existing membership **and** its existing single invoice. The Rider "BIKIE_Sub" confirmation SMS fires once, after the invoice is written, and stamps `MembershipInvoice.confirmationSmsSentAt` only on a non-failing send. Service Provider purchases get an invoice but no SMS (ADR-058); a free-tier activation gets a `₹0` invoice.
...
| `/api/conversations/[id]/messages` | POST | `{ content?, replyToId?, attachments? }` (needs at least one of `content`/`attachments`). Rate-limited (30/min). Returns `{ message }`. ADR-065: `404` (not a distinguishable 403, so probing an id can't confirm it exists) if the caller isn't a participant; `403 { error: "LOCKED" }` if the conversation is locked; `403 { error: "MUTED" }` if the sender is muted. |
