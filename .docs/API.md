...
| `/api/partner/sos/nearby` | GET | ADR-044. `?lat=&lng=` required. `{ requests: PartnerNearbyRequestDTO[] }` — open (unassigned), type-eligible SOS alerts within 25km. Empty (not an error) if the partner is unverified or offline. |
