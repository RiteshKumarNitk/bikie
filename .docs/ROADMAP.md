...
intact — one template per SMS type, no cross-fallback, deprecated `MSG91_TEMPLATE_ID` unused — and
the rendered bodies already match the approved DLT text character-for-character (only the
`##alphanumeric##` slots vary), now locked by exact-match tests. Membership SMS: once, Rider-only,
after payment verification + invoice, deduped. SOS SMS: one body for every recipient, capped
10/batch, skipped safely when unset; RED/EMERGENCY still excludes Service Providers. OTP flow
untouched. Only code change: the SMS adapter now logs and returns MSG91's request id on
acceptance (explicitly *gateway acceptance, not handset delivery* — BIKIE does not ingest MSG91
delivery reports) and classifies a rejection (auth key / sender id / DLT template mismatch /
recipient number / balance) in the failure log. **Open:** Service Provider membership is monthly
but the only membership template is annual — a separate DLT-approved monthly SP template is
required before providers get a purchase SMS; not invented here. See ADR-079.
...
the native `razorpay_flutter` SDK, then the `razorpay_*` triple posted to `/api/*/purchase` for
server-side signature verification. A WebView-hosted first pass was dropped because Razorpay
Checkout hides the UPI-intent option inside a WebView; the native sheet shows UPI apps + UPI ID +
QR alongside every other method. The key id comes from the server; the app embeds no Razorpay key.
Also fixed a 500 on `/api/membership/checkout` (the order `receipt` exceeded Razorpay's 40-char
limit and the throw was unhandled). Operator still needs UPI enabled on the Razorpay account.
Still deferred: the `order.paid` webhook and a plan/amount re-check on purchase. See ADR-075.
...
navigation stack with just the tapped screen. Fixed by always landing on Home first, then pushing
the target on top. Also added a "your SOS alert is active" banner to Home on both platforms — a
rider previously had no way back to an alert they'd sent besides a one-time confirmation button or
a location-gated community list. See ADR-063.
