...
| 4 | SMS (OTP + SOS) | Twilio Programmable Messaging | **Twilio** | Paid | No | `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_MESSAGING_SERVICE_SID` **or** `TWILIO_FROM_NUMBER` / `TWILIO_PHONE_NUMBER` | OTP SMS; SOS SMS to contacts / nearby / partners |
...
| SMS | **Twilio** | MSG91, AWS SNS, etc. (code today = Twilio) |
...
4. **Twilio** — verify number / Messaging Service; send test OTP SMS.
...
|---|---|---|
| Phone OTP signup / login | Twilio + Neon + Better Auth | OTP SMS arrives; session cookie / bearer works |
| Google login | Google OAuth + Neon | Account created/linked; lands as RENTER |
| SOS Red/Amber alert | Twilio + Meta (or Twilio WA) + SMTP/Resend + Neon (+ Upstash preferred) | Alert persisted; SMS/WA/email only on **configured** channels + recipient contact fields; dispatch summary / logs show `channels=sms:…,wa:…,email:…` |
...
|---|---|---|
| P0 | Neon, Better Auth secrets, Twilio SMS, SMTP or Resend, Meta WhatsApp (or Twilio WA), public HTTPS URLs, `MESSAGE_ENCRYPTION_KEY`, `CRON_SECRET` | — |
