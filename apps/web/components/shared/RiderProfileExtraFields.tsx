...
/**
 * Shared field groups for the ADR-014 additions to `RiderProfile` (father/mother
 ...
 *
 * Split into four exported sub-sections (`VehicleDetailsFields`, `RiderPersonalFields`,
 * `GovernmentIdFields`, `RidingDetailsFields`) so a caller can interleave them with its
 * own sections in a different order (see the onboarding form's rider-registration
 * layout) — `RiderProfileExtraFields` itself remains the same combined component in
 * its original order for callers that don't need to reorder (Settings).
 */
...
export interface RiderProfileFieldGroupProps {
...
const defaultSectionHeadingClassName = "text-xs font-semibold uppercase tracking-wider text-accent-text";

export function VehicleDetailsFields({
  value,
  onChange,
  idPrefix = "",
  sectionHeadingClassName = defaultSectionHeadingClassName,
}: RiderProfileFieldGroupProps) {
...
export function RiderPersonalFields({
  value,
  onChange,
  idPrefix = "",
  sectionHeadingClassName = defaultSectionHeadingClassName,
...
export function RidingDetailsFields({
  value,
  onChange,
  idPrefix = "",
  sectionHeadingClassName = defaultSectionHeadingClassName,
...
export function RiderProfileExtraFields(props: RiderProfileFieldGroupProps) {
  return (
    <>
      <div className="border-t border-foreground/10 pt-6">
        <RiderPersonalFields {...props} />
      </div>
      <div className="border-t border-foreground/10 pt-6">
        <VehicleDetailsFields {...props} />
      </div>
      <div className="border-t border-foreground/10 pt-6">
        <GovernmentIdFields {...props} />
      </div>
      <div className="border-t border-foreground/10 pt-6">
        <RidingDetailsFields {...props} />
      </div>
    </>
  );
}
